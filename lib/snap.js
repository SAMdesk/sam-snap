...
class SnapService {
    ...
    searchSnaps(query, snap_fields, done) {
        ...
        client.query(SearchSnaps).then((search_result) => {
            ...
            const network_error = _.get(err, 'networkError', {});

            if(_.isNil(ex)) {
                switch(network_error.code) {
                    case 'ECONNRESET':
                        ex = new SamException(constants.exceptions.connectionReset, 'Connection was reset by Snap API', data, { level: constants.logLevels.warn });
                        break;
                    case 'ETIMEDOUT':
                        ex = new SamException(constants.exceptions.timeout, 'Connection attempt to Snap API timed out', data, { level: constants.logLevels.warn });
                }
            }

            if(_.isNil(ex)) {
                switch(network_error.statusCode) {
                    case 404:
                        ...
                        break;
                    case 500:
                        ex = new SamException(constants.exceptions.remoteServiceError, 'Snap API encountered an unknown error', data, { level: constants.logLevels.warn });
                        break;
                    case 502:
                    case 503:
                        ...
                        break;
                    case 504:
                        ex = new SamException(constants.exceptions.timeout, 'Snap API gateway timed out on request', data, { level: constants.logLevels.warn });
                }
            }
            ...
            if(_.isNil(ex)) {
                switch(err.networkError.name) {
                    case 'ServerParseError':
                        ex = new SamException(constants.exceptions.invalidRequest, 'Snap API encountered an internal error parsing request', data, { level: constants.logLevels.warn });
                        break;
                }
            }

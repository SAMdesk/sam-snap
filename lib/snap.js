...
class SnapService {
    ...
    searchSnaps(query, criteria, done) {
        ...
        client.query(SearchSnapsWithGeoFilter).then((result) => {
            return done(null, (result && result.data && result.data.SearchSnaps) || { snaps: [] });
        }).catch((err) => {
            if(err.message === 'GraphQL error: search request error code:NOT_FOUND  - ') {
                return done(null, { snaps: [] });
            }

            let ex = new SamException('unhandled_error', 'Received unhandled error from Snap API', err, { level: constants.logLevels.error });
            return done(ex);
        });
    ...
    searchStories(query, criteria, done) {
        ...
        client.query(SearchStoriesWithGeoFilter).then((result) => {
            return done(null, _.get(result, 'data.SearchSnaps', { snaps: [] }));
        }).catch((err) => {
            if(err.message === 'GraphQL error: search request error code:NOT_FOUND  - ') {
                return done(null, { snaps: [] });
            }

...
class SnapService {
    ...
    searchSnaps(query, snap_fields, done) {
        ...
        client.query(SearchSnaps).then((search_result) => {
            ...
            let result = _.get(search_result, 'data.SearchSnaps', { snaps: [] });

            if(post_filter) {
                let tmpSnaps = _.filter(snaps.snaps, (snap) => {
                    console.log(query.pagination.min_timestamp, snap.capture_time_secs, query.pagination.max_timestamp);
                    return snap.capture_time_secs >= query.pagination.min_timestamp &&
                        snap.capture_time_secs <= query.pagination.max_timestamp;
                });
                result.snaps = tmpSnaps;
            }

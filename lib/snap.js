...
class SnapService {
    ...
    searchSnaps(query, criteria, done) {
        ...
        if(!_.isNil(criteria.geo) && !_.isEmpty(criteria.geo)) {
            variables.geo_filter = {
                geotype: 'CIRCLE',
                circle: {
                    center: {
        ...
        if(!_.isNil(criteria.terms) && !_.isEmpty(criteria.geo)) {
            variables.content = _.chain(criteria.terms || []).compact().map((x) => {
